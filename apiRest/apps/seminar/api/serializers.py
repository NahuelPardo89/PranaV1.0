...
class SeminarInscriptionSerializer(serializers.ModelSerializer):
    insurance = serializers.PrimaryKeyRelatedField(
        queryset=HealthInsurance.objects.all(), required=False, allow_null=True)

    class Meta:
        model = SeminarInscription
        fields = ['id', 'seminar', 'patient', 'meetingNumber', 'state',
                  'insurance',   'payment', 'created_at', 'updated_at', 'created_by']
