...
export class CreateespecialityComponent {
  ...
  constructor(private fb: FormBuilder, private specialityService:SpecialityService, private dialog:DialogService, private router: Router) {
  ...
  onSubmit(): void {
    ...
    this.specialityForm.get('name')?.setValue(nameInUpperCase);
      this.specialityService.createSpeciality(this.specialityForm.value).subscribe({
        next: (response) => {
